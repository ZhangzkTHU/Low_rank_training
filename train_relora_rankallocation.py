...
parser.add_argument('--min-rank', default=0.5, type=float, help='minimum rank for factorization')
parser.add_argument('--adaptive-metric', default=None, type=str, help='adaptive metric for factorization')
...
if args.net=="vit_small":
    ...
    from models.relora_rankallocation import ReLoRaModel
...
# )
    ...
    from models.relora import ReLoRaModel
    from models.simplevit import SimpleViT
    net = ReLoRaModel(
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        #     if param.shape == torch.Size([768, args.r]):
        #         print(param.grad.norm())
        #         break
        # for name, param in net.named_parameters():
        #     if param.shape == torch.Size([args.r, 128]):
        #         print(param.grad.norm())
        #         break
    ...
    if (epoch+1) % args.merge_freq == 0:
        ...
        ranks = net.merge_and_reinit()
        for group in optimizer.param_groups:
            for p in group['params']:
                optimizer.state[p] = {}
        
        # save ranks in a file
        with open(os.path.join(exp_dir, 'ranks.txt'), 'a') as appender:
            appender.write(f'Epoch {epoch}, ranks: {ranks}\n')
