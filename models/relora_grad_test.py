    ...
    def __init__(
        ...
        *,
        ...
        for module_name, module in self.wrapped_model.named_modules():
            ...
            # print(f'layer {block_idx//4} rank: {128/(2**(3-block_idx//4))}')
            new_module = ReLoRaLinear(
                module.in_features,
                module.out_features,
                bias=module.bias is not None,
                # r=int(128/(2**(3-block_idx//4))),
                r=r,
    ...
    def reinit(self):
        ...
        for module in self.modules():
            if isinstance(module, ReLoRaLinear):
                ...
                module.reinit(VT)
                # module.reinit(projs[block_idx])
                # block_idx += 1


    # def correct_grad(self, correct_coef_A1=0.05, correct_coef_A2=0.95, correct_coef_B1=0.05, correct_coef_B2=0.05):
    #     for module in self.modules():
    #         if isinstance(module, ReLoRaLinear):
    #             module.correct_grad(correct_coef_A1, correct_coef_A2, correct_coef_B1, correct_coef_B2)
...
class ReLoRaLinear(nn.Module):
    ...
    # def correct_grad(self, correct_coef_A1, correct_coef_A2, correct_coef_B1, correct_coef_B2): # a better way of modifying the correct_coefs
        ...
        # self.lora_A.weight.grad.data = correct_coef_A2 * self.lora_A.weight.grad.data
        # self.lora_B.weight.grad.data = correct_coef_B2 * self.lora_B.weight.grad.data
