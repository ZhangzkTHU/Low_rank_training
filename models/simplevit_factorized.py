...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, r=None):
        ...
        self.to_out = nn.Sequential(nn.Linear(inner_dim, r, bias = False), nn.Linear(r, dim, bias = False),)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, r):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Attention(dim, heads = heads, dim_head = dim_head, r=r),
                FeedForward(dim, mlp_dim, r=r)
            ]))
    def forward(self, x):
...
class SimpleViT_Factorized(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels = 3, dim_head = 64, r=None):
        super().__init__()
        ...
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, r)
