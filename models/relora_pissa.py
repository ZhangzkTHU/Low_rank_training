...
class ReLoRaLinear(nn.Module):
    ...
    def merge_and_reinit(self):
        ...
        U, S, VT = torch.linalg.svd(self.weight.data)
        self.lora_A.weight.data = torch.diag(S[:self.r]**0.5) @ VT[:self.r, :]
        self.lora_B.weight.data = U[:, :self.r]@ torch.diag(S[:self.r]**0.5)
        self.weight.data = self.weight.data - self.lora_B.weight.data @ self.lora_A.weight.data
