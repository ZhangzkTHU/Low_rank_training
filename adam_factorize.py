...
class Adam_factor(Adam): 
	def __init__(self, params, rank, lr=1e-3, betas=(0.9, 0.999), weight_decay=0):
	...
	def step(self):
		for group in self.param_groups: 
			for p in group['params']:
				...
				if p.size() in [torch.Size([128, 256]), torch.Size([768, 128]), torch.Size([128, 128])]:
					U, S, VT = torch.linalg.svd(grad)
					grad = torch.diag(S[:self.r]) @ VT[:self.r, :]
				...
				if len(state) == 0:
					...
					# if p.size() in [torch.Size([128, 256]), torch.Size([768, 128]), torch.Size([128, 128])]:
					# 	state["exp_avg"] = [torch.zeros_like(grad_B), torch.zeros_like(grad_A)]
					# # Exponential moving average of squared gradient values 
					# 	state["exp_avg_sq"] = [torch.zeros_like(grad_B), torch.zeros_like(grad_A)]
					# else:
			...
			if p.size() in [torch.Size([128, 256]), torch.Size([768, 128]), torch.Size([128, 128])]:
				...
				p.data.add(U.mm(denom), alpha=-step_size)
