...
if args.net=="vit_small":
    ...
    from models.relora_chagingr import ReLoRaModel
...
def train(epoch):
    ...
    rank = int(args.r + (8-args.r) * ((epoch+1) // args.merge_freq) / (args.n_epochs // args.merge_freq))
    if (epoch+1) % args.merge_freq == 0:
        print("Merging and Reinitializing")
        # choose r by interploating between args.r and 8
        
        net.merge_and_reinit(rank)
        # delete momentum in optimizer
        for group in optimizer.param_groups:
            for p in group['params']:
                optimizer.state[p] = {}
    
    content = time.ctime() + ' ' + f'Epoch {epoch}, rank:{rank}, lr: {optimizer.param_groups[0]["lr"]:.7f}, train loss: {train_loss/(batch_idx+1):.5f}, acc: {(100.*correct/total):.5f}'
    print(content)
