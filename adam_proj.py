...
# 				# proj = projs[self.layer]
# 				# # TODO: find the layer by counting the attention matrix
# 				# if p.size() == torch.Size([2304, 768]):
# 				# 	self.layer = (self.layer+1) % 12

# 				proj = projs[self.block_idx]
# 				if p.size() in [torch.Size([768, 3072]), torch.Size([3072, 768]), torch.Size([2304, 768]), torch.Size([768, 768])]:
# 					self.block_idx = (self.block_idx+1) % 48

# 				# project only large grads
# 				if p.size() in [torch.Size([768, 3072]), torch.Size([3072, 768]), torch.Size([2304, 768]), torch.Size([768, 768])]:
# 					if grad.size(dim=-1) == proj.size(dim=-1):
# 						grad = grad.mm(proj.T) # grad: (dim_out, dim) @ proj.T: (dim, r) -> (dim_out, r)
# 						# print(grad.size())
# 					else:
# 						grad = proj.mm(grad) # proj: (r, dim) @ grad: (dim, dim_out) -> (r, dim_out)
# 				state = self.state[p]
...
# 				if p.size() in [torch.Size([768, 3072]), torch.Size([3072, 768]), torch.Size([2304, 768]), torch.Size([768, 768])]:
# 					if p.size(dim=-1) == proj.size(dim=-1):
# 						p.data.add_(exp_avg.div_(denom).mm(proj), alpha=-step_size)
# 						# p.data.addcdiv_(exp_avg, denom, value=-step_size)
# 					else:
# 						p.data.add_(proj.T.mm(exp_avg.div_(denom)), alpha=-step_size)
# 						# p.data.addcdiv_(exp_avg.mm(proj.T).T, denom, value=-step_size)
# 				else:
# 					p.data.addcdiv_(exp_avg, denom, value=-step_size)
